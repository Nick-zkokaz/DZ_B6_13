import os           # в файле по рабору ДЗ нет такой строки НУЖНО УСТАНОВИТЬ ПАКЕТ
import json         # в файле по рабору ДЗ нет такой строки НУЖНО УСТАНОВИТЬ ПАКЕТ
import datetime     # в файле по рабору ДЗ нет такой строки НУЖНО УСТАНОВИТЬ ПАКЕТ
...
@route("/albums/<artist>")
def albums(artist):
    albums_list = album.find_art(artist) # d
    if not albums_list:
        ...
        result = "Количество альбомов : {}.<BR> Список альбомов {}: <BR>".format(len(album_names),artist)
        result += "<br>".join(album_names)
    return result

@route("/albums", method="POST") # Для проверки работоспособности предлагаю ввести в терминале строку, указанную ниже
# Вызов из терминала  http -f POST localhost:8080/albums year=1990 artist=Aquarium genre=folk album=RadioLondon
# Название альбомов обязательно без пробелов
def user():
    """
    ...
    cash_record = {
        "year": int(request.forms.get("year")), #Пытаемся сделать введенный год в формате int()
        "artist": request.forms.get("artist"),
        "genre": request.forms.get("genre"),
        "album": request.forms.get("album")
    }
    # Проверяем введенный год пользователем.
    if 1000 <= cash_record['year'] <= d.year:  # возможный диапазон от 1000 года до текущего года  (включительно) ТИП только INT
        pass
    else:
